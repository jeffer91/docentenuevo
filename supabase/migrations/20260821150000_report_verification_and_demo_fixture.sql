-- SIACD · Informes ejecutivos, verificación pública y fixture controlado de la docente DEMO.
-- El fixture solo puede ejecutarse sobre docentes cuyo nombre contenga DEMO y no elimina datos reales.
...
as $$
  select case
    when gd.id is null then jsonb_build_object(
      'valid', false,
  ...
  from (select nullif(trim(p_code), '') as code) q
  left join public.generated_documents gd
    on gd.verification_code = q.code
  left join public.expedients e on e.id = gd.expedient_id
