-- SIACD · Unificación del modelo activo de acompañamiento
-- Fuente operativa única: catálogo activo de criterios organizado en
-- Áreas, Antes, Durante y Después. No modifica seguridad ni autenticación.
...
with op_counts as (
  select
    ...
    count(cs.competency_id) filter (
      where cd.hito_id = 'H1'
        and (cs.score is not null or coalesce(cs.not_applicable, false))
...
evidence_stats as (
  select
    e.id as expedient_id,
    count(distinct ev.hito_id) filter (
      where ev.hito_id in ('H1','H2','H3','H4','H5','H6')
    )::integer as evidence_hitos
  from public.expedients e
...
evidence_request_stats as (
  select
    e.id as expedient_id,
    count(er.id) filter (
      where er.status in ('pending','submitted','in_review')
    )::integer as pending_evidence,
    count(er.id) filter (
      where er.status = 'correction_required'
    )::integer as correction_evidence
  from public.expedients e
  left join public.evidence_requests er
    on er.expedient_id = e.id
  ...
  select
    ...
    case
      when count(rr.id) filter (where rr.score is not null and not rr.not_applicable) > 0
        then round(avg(rr.score) filter (where rr.score is not null and not rr.not_applicable) * 25, 1)
      else null::numeric
    end as percent
...
review_stats as (
  select
    ...
    count(cp.id) filter (
      where cp.status in ('planned','open') and cp.scheduled_on < current_date
    )::integer as reviews_overdue,
    min(cp.scheduled_on) filter (
      where cp.status in ('planned','open') and cp.scheduled_on >= current_date
    ) as next_review,
    (
      select c1.percent
      from cycle_percent c1
      where c1.expedient_id = e.id
        and c1.status = 'closed'
        and c1.percent is not null
      order by c1.sequence
      limit 1
    ) as first_review_percent,
    (
      select c2.percent
      from cycle_percent c2
      where c2.expedient_id = e.id
        and c2.status = 'closed'
        and c2.percent is not null
      order by c2.sequence desc
      limit 1
    ) as last_review_percent
  from public.expedients e
...
select
  ...
  coalesce(oc.operational_total, 0)::integer as operational_total,
  coalesce(oc.operational_resolved, 0)::integer as operational_resolved,
  coalesce(os.operational_percent, 0::numeric) as operational_percent,
  coalesce(oc.passed, 0)::integer as passed,
  coalesce(oc.failed, 0)::integer as failed,
  coalesce(oc.not_applicable, 0)::integer as not_applicable,
  coalesce(oc.critical_gaps, 0)::integer as critical_gaps,
  coalesce(ers.pending_evidence, 0)::integer as pending_evidence,
  coalesce(ers.correction_evidence, 0)::integer as correction_evidence,
  coalesce(rs.reviews_closed, 0)::integer as reviews_closed,
  coalesce(rs.reviews_open, 0)::integer as reviews_open,
  coalesce(rs.reviews_overdue, 0)::integer as reviews_overdue,
  rs.next_review,
  ...
  0::integer as complementary_evaluated,
  0::numeric as complementary_percent,
  0::integer as quality_evaluated,
  0::numeric as quality_percent,
  0::integer as quality_critical_gaps,
  coalesce(fs.followups, 0)::integer as followups,
  coalesce(es.evidence_hitos, 0)::integer as evidence_hitos,
  ...
  (
    ...
    and coalesce(os.operational_percent, 0) >= 75
    and coalesce(oc.critical_gaps, 0) = 0
    and coalesce(ers.pending_evidence, 0) = 0
    and coalesce(ers.correction_evidence, 0) = 0
  ) as ready_to_certify
from public.expedients e
  ...
  select jsonb_build_object(
    'summary', jsonb_build_object(
      ...
      'needs_attention', count(*) filter (
        where critical_gaps > 0 or correction_evidence > 0 or reviews_overdue > 0
      ),
    ...
    'rows', coalesce(jsonb_agg(jsonb_build_object(
      ...
      case when critical_gaps > 0 or correction_evidence > 0 or reviews_overdue > 0 then 0 else 1 end,
      teacher_name), '[]'::jsonb)
  )
  from scoped;
$$;
...
comment on view private.expedient_indicator_snapshot is
  'Indicadores SIACD basados exclusivamente en el catálogo activo Áreas/Antes/Durante/Después. Las tablas complementarias y de calidad quedan como histórico y no intervienen en el cálculo activo.';
