...
type Summary = {
  expedients: number; teachers: number; before: number; during: number; after: number;
...
type Row = {
  ...
  phase: "before" | "during" | "after"; operational_resolved: number; operational_total: number;
  ...
  next_review: string | null; improvement_points: number | null; complementary_percent: number; quality_percent: number;
  final_percent: number | null; ready_to_certify: boolean; certified: boolean; classification: string;
};

type Data = { summary: Summary; rows: Row[] };

const emptySummary: Summary = {
  expedients:0, teachers:0, before:0, during:0, after:0, passed:0, failed:0, not_applicable:0,
  critical_gaps:0, pending_evidence:0, corrections:0, reviews_overdue:0, reviews_closed:0,
  certified:0, ready_to_certify:0, needs_attention:0, operational_average:0, improvement_average:0,
};

function phaseLabel(value: Row["phase"]) {
  return value === "before" ? "Antes" : value === "during" ? "Durante" : "Después";
}
...
function tone(value: string) {
  if (value === "Certificado" || value === "Listo para certificar" || value === "Cumple") return styles.good;
  ...
  return <div className={styles.backdrop}>
    <section className={styles.panel} role="dialog" aria-modal="true" aria-label="Indicadores SIACD">
      <header className={styles.header}>
        <div><span>SIACD · BLOQUE 6</span><h2>{mode === "admin" ? "Indicadores institucionales" : "Indicadores de coordinación"}</h2><p>Pasa / no pasa, brechas, revisiones, evidencias, evolución y cierre.</p></div>
      ...
      {loading ? <div className={styles.loading}>Calculando indicadores…</div> : <>
        <div className={styles.metrics}>
          <Metric icon={BarChart3} label="Resultado operativo" value={`${data.summary.operational_average}%`} note={`${data.summary.expedients} expedientes`} />
        ...
        <div className={styles.phaseGrid}>
          ...
          <div><strong>{data.summary.passed}</strong><span>Pasan</span></div>
          <div><strong>{data.summary.failed}</strong><span>No pasan</span></div>
        ...
        <div className={styles.tableWrap}><table className={styles.table}>
          <thead><tr><th>Docente</th><th>Momento</th><th>Resultado</th><th>Pasa / No pasa</th><th>Brechas</th><th>Evidencias</th><th>Revisión</th><th>Evolución</th><th>Estado</th></tr></thead>
          <tbody>{filtered.map((row) => <tr key={row.expedient_id}>
            ...
            <td><strong>{row.operational_percent}%</strong><small>{row.operational_resolved}/{row.operational_total} resueltos</small></td>
            ...
            <td><span className={`${styles.state} ${tone(row.classification)}`}>{row.classification}</span>{row.final_percent !== null && <small>Final {row.final_percent}%</small>}</td>
          </tr>)}</tbody>
        </table></div>
