...
const tabLabels: Record<WorkspaceTab, string> = {
  ...
  complementary: "Matriz complementaria",
  quality: "Calidad",
  evidence: "Evidencias",
...
export default function ExpedientWorkspace({
  ...
  const load = useCallback(async () => {
    ...
    const anyError =
      hitoResult.error || scheduleResult.error || competencyResult.error || scoreResult.error || followupResult.error || actionResult.error;
    if (anyError) {
  ...
  const hitoSummary = useMemo(() => {
    ...
    for (const hito of hitos) {
      ...
      const verdict =
        completed === 0 ? "SIN EVALUAR" : criticalGaps > 0 ? "REQUIERE ACOMPAÑAMIENTO" : (average ?? 0) >= 3 ? "HITO SUPERADO" : "EN DESARROLLO";
      ...
      <section className={styles.workspace} role="dialog" aria-modal="true" aria-label={`Expediente de ${teacher.name}`}>
        <header className={styles.header}>
          <div className={styles.headerIdentity}>
            ...
            <div>
              <div className={styles.eyebrow}>EXPEDIENTE DOCENTE · SIACD INTEGRAL</div>
              <h2>{teacher.name}</h2>
        ...
        <div className={styles.content}>
          {tab === "summary" && (
            <SummaryTab teacher={teacher} coordinatorName={coordinatorName} hitos={hitos} summaries={hitoSummary} operationalPercent={operationalPercent} totalCriticalGaps={totalCriticalGaps} />
          )}

          {tab === "schedule" && (
            <ScheduleTab hitos={hitos} schedules={schedules} setSchedules={setSchedules} summaries={hitoSummary} savingId={savingId} onSave={saveHito} />
          )}
          ...
          {tab === "log" && (
            <LogTab followups={followups} hitos={hitos} showForm={showFollowupForm} setShowForm={setShowFollowupForm} onAdd={addFollowup} onDelete={deleteFollowup} coordinatorName={coordinatorName} />
          )}

          {tab === "improvement" && (
            <ImprovementTab actions={actions} competencies={competencies} lowCompetencies={lowCompetencies} scores={scores} showForm={showActionForm} setShowForm={setShowActionForm} onAdd={addAction} onStatus={setActionStatus} onDelete={deleteAction} coordinatorName={coordinatorName} />
          )}
          {(["complementary","quality","evidence","documents","certification"] as FinalizationMode[]).includes(tab as FinalizationMode) && (
            <ExpedientFinalization
              mode={tab as FinalizationMode}
              teacher={teacher}
              accessMode={accessMode}
              coordinatorName={coordinatorName}
              operationalPercent={operationalPercent}
              operationalEvaluated={evaluatedCount}
              operationalCriticalGaps={totalCriticalGaps}
              onChanged={onChanged}
            />
          )}
        </div>
...
function SummaryTab({
  teacher, coordinatorName, hitos, summaries, operationalPercent, totalCriticalGaps,
}: {
  teacher: Teacher;
  coordinatorName: string;
  hitos: HitoDefinition[];
  summaries: Record<string, { average:number|null;criticalGaps:number;completed:number;total:number;verdict:string }>;
  ...
  return <div className={styles.summaryGrid}>
    <section className={styles.card}><h3>Ficha del docente</h3><dl className={styles.definitionList}>
      <div><dt>Docente</dt><dd>{teacher.name}</dd></div>
      <div><dt>Carrera</dt><dd>{teacher.career}</dd></div>
      <div><dt>Asignatura(s)</dt><dd>{teacher.subject}</dd></div>
      <div><dt>Modalidad</dt><dd>{teacher.modality}</dd></div>
      <div><dt>Período</dt><dd>{teacher.period}</dd></div>
      <div><dt>Coordinador</dt><dd>{coordinatorName}</dd></div>
      <div><dt>Fecha de ingreso</dt><dd>{formatDate(teacher.entryDate)}</dd></div>
      <div><dt>Inicio de actividades</dt><dd>{formatDate(teacher.activitiesStartDate)}</dd></div>
      <div><dt>Cierre previsto</dt><dd>{formatDate(teacher.plannedCloseDate)}</dd></div>
      <div><dt>Jornada(s)</dt><dd>{teacher.scheduleRanges.join(" · ") || "—"}</dd></div>
    ...
    <section className={`${styles.card} ${styles.wide}`}><h3>Resumen de H1–H6</h3><div className={styles.hitoCards}>{hitos.map((hito)=>{
...
function ScheduleTab({
  hitos, schedules, setSchedules, summaries, savingId, onSave,
}: {
  hitos:HitoDefinition[];
...
function HitoTab({
  ...
  summary?:{average:number|null;criticalGaps:number;completed:number;total:number;verdict:string};
  savingId:string;
...
function LogTab({
  ...
  return <><section className={styles.card}><div className={styles.sectionHead}><div><h3>Bitácora de seguimiento</h3><p>Registro cronológico de intervenciones, hallazgos y compromisos.</p></div><button className={styles.primary} onClick={()=>setShowForm(!showForm)}><Plus size={14}/>{showForm?"Cerrar":"Nuevo seguimiento"}</button></div>
    {showForm&&<form className={styles.formGrid} onSubmit={onAdd}>
      <label>Fecha<input name="happened_on" type="date" required defaultValue={today()}/></label>
      <label>Hito<select name="hito_id"><option value="">General</option>{hitos.map((h)=><option key={h.id} value={h.id}>{h.id} · {h.title}</option>)}</select></label>
      <label>Tipo de seguimiento<input name="followup_type" required placeholder="Revisión, tutoría, reunión..."/></label>
      <label>Proceso / plataforma<input name="process" placeholder="EVA, SISACAD, Teams..."/></label>
      <label className={styles.full}>Hallazgo o competencia verificada<textarea name="finding" required rows={3}/></label>
      <label className={styles.full}>Compromiso / acción acordada<textarea name="agreed_action" rows={2}/></label>
      <label>Fecha compromiso<input name="commitment_due_on" type="date"/></label>
      <label>Responsable<input name="responsible" defaultValue={coordinatorName}/></label>
      <label className={styles.full}>Evidencia / enlace / código<input name="evidence_reference" placeholder="URL, código o referencia"/></label>
      <label className={styles.checkbox}><input type="checkbox" name="teacher_conformity"/>Conformidad del docente</label>
      <div className={styles.formActions}><button type="button" className={styles.secondary} onClick={()=>setShowForm(false)}>Cancelar</button><button className={styles.primary}><Save size={14}/>Guardar seguimiento</button></div>
    </form>}
...
function ImprovementTab({
  actions,competencies,lowCompetencies,scores,showForm,setShowForm,onAdd,onStatus,onDelete,coordinatorName,
}:{
  actions:ImprovementAction[];competencies:CompetencyDefinition[];lowCompetencies:CompetencyDefinition[];scores:Record<string,ScoreRecord>;
  showForm:boolean;setShowForm:(value:boolean)=>void;onAdd:(event:FormEvent<HTMLFormElement>)=>Promise<void>;
  onStatus:(action:ImprovementAction,status:ImprovementAction["status"])=>Promise<void>;onDelete:(id:string)=>Promise<void>;coordinatorName:string;
}) {
  const defMap=Object.fromEntries(competencies.map((d)=>[d.id,d]));
  return <section className={styles.card}><div className={styles.sectionHead}><div><h3>Plan individual de mejora</h3><p>Las brechas se detectan con puntajes menores a 3. Incluye todos los criterios del Excel.</p></div><button className={styles.primary} disabled={!lowCompetencies.length} onClick={()=>setShowForm(!showForm)}><Plus size={14}/>{showForm?"Cerrar":"Nueva acción"}</button></div>
    <div className={styles.breachStrip}><strong>{lowCompetencies.length}</strong><span>competencias con brecha evaluada</span>{lowCompetencies.length===0&&<em>No hay brechas con los puntajes actuales.</em>}</div>
    {showForm&&<form className={styles.formGrid} onSubmit={onAdd}>
      <label className={styles.full}>Competencia con brecha<select name="competency_id" required>{lowCompetencies.map((d)=><option key={d.id} value={d.id}>{d.id} · {d.observable_competency}</option>)}</select></label>
      <label className={styles.full}>Acción de mejora<textarea name="action_text" required rows={3}/></label>
      <label>Responsable<input name="responsible" required defaultValue={coordinatorName}/></label>
      <label>Fecha compromiso<input type="date" name="due_on"/></label>
      <div className={styles.formActions}><button type="button" className={styles.secondary} onClick={()=>setShowForm(false)}>Cancelar</button><button className={styles.primary}><Save size={14}/>Crear acción</button></div>
    </form>}
    {actions.length?<div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Hito / ID</th><th>Competencia</th><th>Puntaje</th><th>Criticidad</th><th>Acción</th><th>Responsable</th><th>Fecha</th><th>Estado</th><th></th></tr></thead><tbody>{actions.map((action)=>{
