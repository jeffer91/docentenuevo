...
type ComponentSummary = Summary & { name: string };
...
function componentSummaries(rows: CriterionRow[]): ComponentSummary[] {
  ...
  return [...grouped.entries()].map(([name, items]) => ({ name, ...summarize(items) }));
}
...
function statusDonut(summary: Summary) {
  const prepared = makeCanvas(1000, 430);
  if (!prepared) return "";
  ...
  items.forEach(([, value, color]) => {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, 135, angle, angle + slice);
    ctx.closePath();
    ctx.fillStyle = color;
    ctx.fill();
    angle += slice;
...
function componentBarChart(components: ComponentSummary[], title: string) {
  const data = components.slice(0, 14);
  const prepared = makeCanvas(1200, Math.max(330, 115 + data.length * 58));
  if (!prepared) return "";
  const { canvas, ctx } = prepared;
  ctx.fillStyle = "#173f62";
  ctx.font = "bold 28px Arial";
  ctx.fillText(title, 32, 43);
  let y = 95;
...
function phaseBarChart(rows: CriterionRow[]) {
  ...
  ctx.fillText("Cumplimiento por etapa", 32, 44);
  data.forEach((item, index) => {
    const value = item.summary.compliance ?? 0;
    const x = 115 + index * 205;
    const h = 205 * value / 100;
    ctx.fillStyle = "#edf1f4";
    ctx.fillRect(x, 105, 86, 205);
    ctx.fillStyle = item.summary.compliance === null ? "#b7c0c9" : value >= 80 ? "#2e7d5b" : value >= 60 ? "#d09020" : "#c15b37";
    ctx.fillRect(x, 310 - h, 86, h);
    ctx.textAlign = "center";
    ctx.fillStyle = "#173f62";
    ctx.font = "bold 19px Arial";
    ctx.fillText(item.summary.compliance === null ? "—" : `${value}%`, x + 43, 94);
    ctx.font = "17px Arial";
    ctx.fillText(item.label, x + 43, 344);
  });
  ctx.textAlign = "left";
  return canvas.toDataURL("image/png");
}

function conclusionText(summary: Summary, components: ComponentSummary[]) {
  ...
  if (summary.official) return `La evaluación se encuentra completa. Todos los criterios aplicables fueron evaluados y aprobados, con un cumplimiento evaluado del ${summary.compliance ?? 100} %. El informe cumple las condiciones para emitirse como documento oficial.`;
  return `El avance de evaluación es ${summary.advance} % y el cumplimiento de los criterios evaluados es ${summary.compliance ?? 0} %.${best ? ` El componente con mejor desempeño es ${best.name}.` : ""}${attention ? ` La principal atención se concentra en ${attention.name}.` : ""} El proceso continúa en estado borrador.`;
}
...
export default function FormalReportWorkspaceV2({ teacher, accessMode, coordinatorName, onClose }: { teacher: Teacher; accessMode: AccessMode; coordinatorName: string; onClose: () => void }) {
    ...
    try {
      ...
      const section = (title: string, subtitle?: string) => {
        ...
        if (subtitle) {
          ...
          pdf.setFont("helvetica", "normal");
          pdf.setFontSize(7.2);
          pdf.text(pdf.splitTextToSize(subtitle, contentWidth - 8), margin + 4, y + 7);
          y += 14;
        } else y += 9;
        spacer(4);
      };
      const card = (x: number, top: number, width: number, height: number, label: string, value: string, color: [number, number, number]) => {
        ...
        pdf.setTextColor(...color);
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(value.length > 18 ? 8.2 : 11.8);
        pdf.text(value, x + 4, top + 8);
      ...
      const callout = (title: string, lines: string[], tone: "blue" | "amber") => {
        ...
        const wrapped = all.flatMap((text, index) => pdf.splitTextToSize(text, index === 0 ? contentWidth - 10 : contentWidth - 14));
        const height = Math.max(20, wrapped.length * 4 + 9);
        ensure(height + 6);
      ...
      const drawHeader = () => {
        ...
        pdf.text(pdf.splitTextToSize(definition.subtitle, 134), margin, y);
        ...
        pdf.roundedRect(164, 25, 32, 10, 2, 2, "F");
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(7.5);
        pdf.text(official ? "OFICIAL" : "BORRADOR", 180, 31.4, { align: "center" });
        y += 13;
        pdf.setDrawColor(218, 226, 232);
        pdf.roundedRect(margin, y, contentWidth, 35, 2, 2, "S");
        ...
        meta("Docente", teacher.name, 18, y + 7, 76);
        meta("Carrera", teacher.career, 102, y + 7, 88);
        meta("Asignatura", teacher.subject, 18, y + 22, 76);
        meta("Período / modalidad", `${teacher.period} · ${teacher.modality}`, 102, y + 22, 88);
        y += 41;
      };
      ...
      const drawExecutive = (reportRows: CriterionRow[], label: string) => {
        ...
        section("Gráficos", "Lectura visual del estado y del cumplimiento por componente.");
        const donut = statusDonut(current);
        if (donut) {
          ensure(76);
          pdf.addImage(donut, "PNG", margin + 8, y, contentWidth - 16, 68);
          y += 73;
        }
        const bars = definition.key === "informe_consolidado" ? phaseBarChart(reportRows) : componentBarChart(components, definition.phase === "during" ? "Cumplimiento por bloque" : "Cumplimiento por componente");
        if (bars) {
          ensure(82);
          pdf.addImage(bars, "PNG", margin + 4, y, contentWidth - 8, 76);
          y += 81;
        }

        section("Conclusiones");
        callout("Conclusión del informe", [conclusionText(current, components)], "blue");
        section("Aspectos por mejorar");
        callout("Prioridades de mejora", improvementItems(current, components), "amber");
        if (label) {
          const analysis = current.evaluated === 0
            ? `En ${label} todavía no existen criterios evaluados. El avance es ${current.advance}% y el cumplimiento permanece como Sin evaluación.`
            : `En ${label} se han evaluado ${current.evaluated} de ${current.applicable} criterios aplicables. El avance es ${current.advance}% y el cumplimiento evaluado es ${current.compliance ?? 0}%.`;
      ...
      const drawCriterion = (row: CriterionRow) => {
        ...
        const titleLines = pdf.splitTextToSize(row.definition.observable_competency, 132);
        const observation = row.score?.coordinator_observation?.trim() || "";
        const observationLines = observation ? pdf.splitTextToSize(`Observación: ${observation}`, 169) : [];
        const evidence: string[] = [];
        (row.latest?.items ?? []).forEach((item) => {
          const ref = item.kind === "link" ? item.external_url : item.file_name;
          if (ref) evidence.push(item.kind === "link" ? `Tipo de evidencia: Enlace · ${ref}` : `Evidencia: ${ref}`);
        });
        if (row.latest?.submitted_at) evidence.push(`Fecha de carga: ${formatDate(row.latest.submitted_at)}`);
        const reviewDate = row.latest?.reviewed_at || row.score?.evaluated_at;
        if (reviewDate) evidence.push(`Fecha de revisión: ${formatDate(reviewDate)}`);
        if (evaluator) evidence.push(`Evaluador: ${evaluator.full_name} · ${roleLabel(evaluator.role)}`);
        const traceLines = evidence.flatMap((text) => pdf.splitTextToSize(text, 169));
        const boxHeight = Math.max(22, 13 + titleLines.length * 3.6 + observationLines.length * 3.3 + traceLines.length * 3.1 + (traceLines.length ? 4 : 0));
        ensure(boxHeight + 7);
        const top = y;
        ...
        pdf.setFontSize(6.7);
        pdf.text(row.state.label.toUpperCase(), 192, top + 7, { align: "right" });
        let yy = top + 9 + titleLines.length * 3.6;
        ...
        if (observationLines.length) {
          ...
          yy += observationLines.length * 3.3 + 2;
        }
      ...
      const drawDetails = (reportRows: CriterionRow[], annex = false) => {
        newPage();
        section(annex ? "Anexo · Detalle completo" : "Detalle de criterios", annex ? "Trazabilidad completa para auditoría." : "Estado, calificación, evidencia, fechas y evaluador.");
          ...
          for (const group of ordered) {
            if (phase === "during") {
              ...
              if (next !== family) {
                family = next;
                spacer(5);
                line(family, 9.2, true, [80, 66, 126]);
              }
            }
            spacer(3);
            line(group, 8.8, true, [35, 75, 107]);
      ...
      const drawHistory = () => {
        const closed = reviewCycles.filter((item) => item.status === "closed" && item.percent !== null).sort((a, b) => a.sequence - b.sequence).slice(-6);
        if (closed.length < 2) return;
        section("Evolución histórica");
        const historyItems = closed.map((item) => `Revisión ${item.sequence}: ${item.percent}%`);
        callout("Evolución del cumplimiento", historyItems, "blue");
      };
      ...
      const drawClosure = async () => {
        ...
        if (evaluators.length) line(`Evaluador(es): ${evaluators.slice(0, 4).map((item) => `${item.full_name} (${roleLabel(item.role)})`).join(" · ")}`, 7.6);
        line(`Generado por: ${generatorStaff?.full_name || (accessMode === "admin" ? "Administrador SIACD" : coordinatorName || "Coordinación académica")}`, 7.6);
        line(`Fecha de generación: ${formatDate(ecuadorToday())} · Versión ${version}`, 7.5);
        line(`Estado: ${official ? "INFORME OFICIAL" : "BORRADOR"}`, 7.7, true, official ? colors.approved : colors.correction);
        line(`Código de verificación: ${code}`, 7.4, true);
        ensure(34);
        ...
        if (qr) {
          ...
          pdf.text("Escanee para verificar autenticidad", 48, y + 14);
          y += 33;
        } else {
          line(`Verificación: ${verificationUrl}`, 6.5);
        }
      };
  ...
  return <div style={{ position: "fixed", inset: 0, background: "rgba(8,22,38,.58)", zIndex: 10020, display: "grid", placeItems: "center", padding: 18 }}>
    <section style={{ width: "min(940px,96vw)", maxHeight: "92vh", overflow: "auto", background: "white", borderRadius: 18, boxShadow: "0 24px 70px rgba(0,0,0,.24)" }}>
      <header style={{ padding: "20px 22px", background: "#0d2946", color: "white", display: "flex", justifyContent: "space-between", gap: 20, alignItems: "flex-start" }}>
        <div><span style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.2 }}>DOCUMENTACIÓN INSTITUCIONAL</span><h2 style={{ margin: "5px 0 2px" }}>Informes SIACD</h2><p style={{ margin: 0, opacity: .78, fontSize: 13 }}>Resumen ejecutivo, gráficos, conclusiones y trazabilidad.</p></div>
