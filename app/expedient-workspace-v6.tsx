...
function evaluationTone(record?: ScoreRecord) {
  if (!record || (record.score === null && !record.not_applicable)) return styles.pending;
  if (record.not_applicable) return styles.neutral;
  if ((record.score ?? 0) >= 3) return styles.good;
  if (record.score === 2) return styles.warning;
...
export default function ExpedientWorkspaceV6({ teacher, accessMode, coordinatorName, onClose, onChanged }: {
  teacher: Teacher;
  ...
  const load = useCallback(async () => {
    ...
    if (!loadedCriteria.some((item) => item.id.startsWith("AR-"))) {
      setSchemaWarning("El código ya usa la nueva organización, pero el catálogo de Supabase aún es el anterior. Aplique la migración 20260820110000_new_accompaniment_structure.sql para cargar los criterios de Áreas, Antes, Durante y Después.");
    }
  ...
  const phaseSummaries = useMemo(() => {
    ...
    for (const phase of Object.keys(phaseHitos) as PhaseKey[]) {
      ...
      const progress = defs.length ? Math.round((resolved / defs.length) * 100) : 0;
      const status: PhaseSummary["status"] = resolved === 0
        ? "No iniciado"
        : criticalGaps > 0
          ? "En mejora"
          : resolved === defs.length && defs.length > 0 && percent >= 75
            ? "Cumple"
            : "En proceso";
      result[phase] = { percent, progress, resolved, total: defs.length, applicable, criticalGaps, status };
    }
    return result;
  }, [criteria, scores]);
  ...
  async function saveCriterion(def: CompetencyDefinition) {
    ...
    const { error } = await supabase.from("competency_scores").upsert({
      ...
      competency_id: def.id,
      score: current.not_applicable ? null : current.score,
    ...
    if (error) {
      ...
      setScores((state) => ({ ...state, [def.id]: { ...current, evaluated_at: resolved ? new Date().toISOString() : null } }));
      setMessage("Criterio guardado correctamente");
      await onChanged();
    }
    setSavingId("");
  }
  ...
  async function generateReport(reportKey: ReportKey) {
    ...
    try {
      ...
      const addPhaseDetail = (phase: PhaseKey) => {
        ...
        text(`Avance: ${summary.resolved}/${summary.total} criterios (${summary.progress}%). Brechas críticas: ${summary.criticalGaps}. Evidencias registradas: ${evidenceCountForPhase(phase)}.`, 8.5);
      ...
      if (definition.phase) {
        ...
        text(`Cumplimiento global: ${globalSummary.percent}%. Avance: ${globalSummary.resolved}/${globalSummary.total} criterios (${globalSummary.progress}%). Brechas críticas: ${globalSummary.criticalGaps}.`, 9);
        for (const phase of ["areas", "before", "during", "after"] as PhaseKey[]) {
          const summary = phaseSummaries[phase];
          text(`${phaseLabels[phase].title}: ${summary.percent}% · ${summary.status} · ${summary.resolved}/${summary.total} criterios.`, 9, true);
        }
      ...
      setMessage(registered ? `${definition.title} generado, descargado y registrado.` : `${definition.title} descargado. No se pudo registrar en Supabase; revise Storage/migraciones.`);
      ...
      <section className={styles.workspace} role="dialog" aria-modal="true" aria-label={`Expediente de ${teacher.name}`}>
        <header className={styles.header}>
          <div className={styles.identity}>
            ...
            <div>
              <span className={styles.eyebrow}>EXPEDIENTE DOCENTE · ÁREAS / ANTES / DURANTE / DESPUÉS</span>
              <h2>{teacher.name}</h2>
              <p>{teacher.career} · {teacher.subject}</p>
            </div>
          </div>
          <div className={styles.headerStats}>
            <span><b>{globalSummary.progress}%</b><small>avance</small></span>
        ...
        <nav className={styles.mainNav}>
          ...
          {(["areas", "before", "during", "after"] as PhaseKey[]).map((phase) => (
            <button key={phase} className={tab === phase ? styles.active : ""} onClick={() => setTab(phase)}>
              {phaseLabels[phase].title}<span>{phaseSummaries[phase].percent}%</span>
            </button>
          ))}
        ...
        <main className={styles.content}>
          ...
          {(["areas", "before", "during", "after"] as PhaseKey[]).includes(tab as PhaseKey) && (
            <PhaseView
              phase={tab as PhaseKey}
              criteria={criteriaForPhase(tab as PhaseKey)}
              scores={scores}
              setScores={setScores}
              summary={phaseSummaries[tab as PhaseKey]}
              savingId={savingId}
              onSave={saveCriterion}
              evidenceCount={evidenceCountForPhase(tab as PhaseKey)}
            />
          )}
          ...
          {tab === "history" && (
            <div className={styles.historyGrid}>
              <section className={styles.card}>
                ...
                {followups.length === 0 ? <div className={styles.empty}>No existen seguimientos registrados.</div> : (
                  <div className={styles.followupList}>{followups.slice(0, 12).map((item) => (
                    <article key={item.id}><strong>{formatDate(item.happened_on)} · {item.hito_id ?? "General"}</strong><p>{item.finding}</p>{item.agreed_action && <small>Acción: {item.agreed_action}</small>}</article>
                  ))}</div>
                )}
              </section>
              <section className={styles.card}>
                <div className={styles.sectionHead}><div><h3>Evidencias</h3><p>Revisión y control de evidencias del expediente.</p></div></div>
...
function SummaryView({ teacher, phaseSummaries, globalSummary, onOpen }: {
  ...
  globalSummary: { percent: number; progress: number; resolved: number; total: number; criticalGaps: number };
  ...
  return (
    <div className={styles.summaryRoot}>
      ...
      <section className={styles.globalCard}>
        <div><span>Cumplimiento global</span><strong>{globalSummary.percent}%</strong></div>
        <div className={styles.progressBar}><span style={{ width: `${globalSummary.progress}%` }}/></div>
        <p>{globalSummary.resolved}/{globalSummary.total} criterios resueltos · {globalSummary.criticalGaps} brechas críticas.</p>
      </section>
      ...
      <section className={styles.phaseCards}>
        {(["areas", "before", "during", "after"] as PhaseKey[]).map((phase) => {
          ...
          return (
            <button key={phase} className={styles.phaseCard} onClick={() => onOpen(phase)}>
              <div className={styles.phaseCardHead}><div><span>{phaseLabels[phase].title}</span><small>{phaseLabels[phase].description}</small></div><strong>{item.percent}%</strong></div>
              <div className={styles.progressBar}><span style={{ width: `${item.progress}%` }}/></div>
              <div className={styles.phaseMeta}><span>{item.resolved}/{item.total} criterios</span><span>{item.criticalGaps} brechas</span><b>{item.status}</b></div>
            </button>
          );
        })}
      </section>
      ...
      <section className={styles.quickActions}>
        ...
        <button onClick={() => onOpen("history")}><History size={17}/>Revisar historial y evidencias</button>
      </section>
    </div>
  );
}

function PhaseView({ phase, criteria, scores, setScores, summary, savingId, onSave, evidenceCount }: {
  phase: PhaseKey;
  criteria: CompetencyDefinition[];
  scores: Record<string, ScoreRecord>;
  setScores: React.Dispatch<React.SetStateAction<Record<string, ScoreRecord>>>;
  summary: PhaseSummary;
  savingId: string;
  onSave: (def: CompetencyDefinition) => Promise<void>;
  evidenceCount: number;
}) {
  const groups = useMemo(() => {
    const map = new Map<string, CompetencyDefinition[]>();
    for (const def of criteria) {
      const list = map.get(def.process) ?? [];
      list.push(def);
      map.set(def.process, list);
    }
  ...
  function current(def: CompetencyDefinition): ScoreRecord {
    return scores[def.id] ?? {
      competency_id: def.id,
      score: null,
      not_applicable: false,
      coordinator_observation: "",
  ...
  function updateEvaluation(def: CompetencyDefinition, value: string) {
    const row = current(def);
    setScores((state) => ({
      ...state,
      [def.id]: {
        ...row,
        not_applicable: value === "na",
        score: value === "" || value === "na" ? null : Number(value),
      },
    }));
  }

  return (
    <div className={styles.phaseRoot}>
      <section className={styles.phaseIntro}>
        <div><span>ETAPA</span><h3>{phaseLabels[phase].title}</h3><p>{phaseLabels[phase].description}</p></div>
        <div className={styles.metricRow}>
          <span><small>Cumplimiento</small><b>{summary.percent}%</b></span>
          <span><small>Avance</small><b>{summary.progress}%</b></span>
          <span><small>Criterios</small><b>{summary.resolved}/{summary.total}</b></span>
          <span><small>Evidencias</small><b>{evidenceCount}</b></span>
          <span><small>Brechas</small><b>{summary.criticalGaps}</b></span>
          <span><small>Estado</small><b>{summary.status}</b></span>
        </div>
      </section>

      {criteria.length === 0 && <div className={styles.empty}>No existen criterios activos para esta etapa.</div>}

      {groups.map(([group, defs]) => (
        <section className={styles.card} key={group}>
          <div className={styles.sectionHead}>
            <div><h3>{group}</h3><p>{defs.length} criterio{defs.length === 1 ? "" : "s"}</p></div>
          </div>
          <div className={styles.criteriaList}>
            {defs.map((def) => {
              const row = current(def);
              const value = row.not_applicable ? "na" : row.score === null ? "" : String(row.score);
              return (
                <article className={styles.criterion} key={def.id}>
                  <div className={styles.criterionBody}>
                    <div className={styles.criterionTitle}>
                      <span className={def.criticality === "Crítica" ? styles.critical : styles.important}>{def.criticality}</span>
                      <strong>{def.observable_competency}</strong>
                    </div>
                    {def.expected_evidence && <p>Evidencia esperada: {def.expected_evidence}</p>}
                  </div>
                  <div className={styles.criterionControls}>
                    <label>
                      Evaluación
                      <select value={value} onChange={(event) => updateEvaluation(def, event.target.value)}>
                        <option value="">Pendiente</option>
                        <option value="4">4 · Cumple integralmente</option>
                        <option value="3">3 · Cumple</option>
                        <option value="2">2 · En acompañamiento</option>
                        <option value="1">1 · Cumplimiento incipiente</option>
                        <option value="0">0 · No cumple</option>
                        <option value="na">No aplica</option>
                      </select>
                    </label>
                    <span className={`${styles.stateBadge} ${evaluationTone(row)}`}>{evaluationLabel(row)}</span>
                    <label className={styles.observationField}>
                      Observación
                      <textarea
                        rows={2}
                        placeholder="Hallazgo, recomendación o compromiso…"
                        value={row.coordinator_observation}
                        onChange={(event) => setScores((state) => ({ ...state, [def.id]: { ...row, coordinator_observation: event.target.value } }))}
                      />
                    </label>
                    <div className={styles.saveLine}>
                      <small>{row.evaluated_at ? `Última evaluación: ${formatDate(row.evaluated_at)}` : "Sin evaluar"}</small>
                      <button onClick={() => void onSave(def)} disabled={savingId === def.id}><Save size={14}/>{savingId === def.id ? "Guardando…" : "Guardar"}</button>
                    </div>
                  </div>
                </article>
              );
  ...
  return (
    <div className={styles.reportsRoot}>
      <section className={styles.reportsIntro}>
        <div><span>DOCUMENTACIÓN</span><h3>Informes PDF</h3><p>Los cuatro informes de etapa muestran el detalle; el consolidado resume el proceso completo.</p></div>
        <div className={allComplete ? styles.ready : styles.draft}><CheckCircle2 size={17}/><span>{allComplete ? "Expediente completo: consolidado oficial habilitado." : "El consolidado se generará como BORRADOR mientras existan criterios pendientes."}</span></div>
      </section>

      <section className={styles.reportGrid}>
        {reportDefinitions.map((def) => (
          <article className={styles.reportCard} key={def.key}>
            <FileText size={22}/>
            <div><strong>{def.title}</strong><p>{def.description}</p>{def.phase && <small>{phaseSummaries[def.phase].resolved}/{phaseSummaries[def.phase].total} criterios · {phaseSummaries[def.phase].percent}%</small>}</div>
      ...
      <section className={styles.card}>
        ...
        {officialDocuments.length === 0 ? <div className={styles.empty}>Todavía no se han generado informes con la nueva estructura.</div> : (
          <div className={styles.documentList}>{officialDocuments.map((item) => (
            <article key={item.id}>
