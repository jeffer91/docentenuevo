...
type CatalogOption = { id: string; name: string; program?: string };
...
function relationName(value: unknown) {
  ...
  if (!relation || typeof relation !== "object" || !("name" in relation)) return "Sin asignar";
  const name = String(relation.name);
  const program = "program" in relation && relation.program ? String(relation.program) : "";
...
export default function SiacdApp() {
  ...
  useEffect(() => {
    ...
    async function applySession(userId?: string) {
      ...
      const [{ data: profile }, { data: careerRows }, { data: periodRows }, { data: expedientRows }, { data: profileRows }] = await Promise.all([
        ...
        supabase.from("expedients").select("id, status, subject_names, critical_gaps, teachers(full_name), careers(name, program), academic_periods(name), hito_schedules(hito_id, executed_on)").order("created_at", { ascending: false }),
  ...
  async function saveTeacher(input: NewTeacherInput) {
    ...
    const { data: expedient, error: expedientError } = await supabase
      ...
      .select("id, status, subject_names, critical_gaps, teachers(full_name), careers(name, program), academic_periods(name), hito_schedules(hito_id, executed_on)")
      .single();
